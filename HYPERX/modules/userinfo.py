...
from HYPERX.__main__ import STATS, TOKEN, USER_INFO
import HYPERX.modules.sql.userinfo_sql as sql
from HYPERX.modules.disable import DisableAbleCommandHandler
from HYPERX.modules.sql.global_bans_sql import is_user_gbanned
from HYPERX.modules.sql.afk_sql import is_afk, check_afk_status
from HYPERX.modules.sql.users_sql import get_user_num_chats
from HYPERX.modules.helper_funcs.chat_status import sudo_plus
from HYPERX.modules.helper_funcs.extraction import extract_user
from HYPERX import telethn as YoneTelethonClient, TIGERS, DRAGONS, DEMONS
...
def info(update: Update, context: CallbackContext):
    ...
    if user.id == OWNER_ID:
        text += "\n\nThe Disaster level of this person is 'ɢᴏᴅ'."
        disaster_level_present = True
    elif user.id in DEV_USERS:
        text += "\n\nThis user is member of 'ʙɪᴏɴɪᴄ ʀᴏʙᴏᴛs'."
        disaster_level_present = True
    elif user.id in DRAGONS:
        text += "\n\nThe Disaster level of this person is 'ᴅʀᴀɢᴏɴ'."
        disaster_level_present = True
    elif user.id in DEMONS:
        text += "\n\nThe Disaster level of this person is 'ᴅᴇᴍᴏɴ'."
        disaster_level_present = True
    elif user.id in TIGERS:
        text += "\n\nThe Disaster level of this person is 'ᴛɪɢᴇʀ'."
        disaster_level_present = True
    elif user.id in WOLVES:
        text += "\n\nThe Disaster level of this person is 'ᴡᴏʟғ'."
        disaster_level_present = True

    if disaster_level_present:
        text += ' [<a href="https://t.me/HYPERx_UPDATES/2">?</a>]'.format(
            bot.username
        )
...
*What is that health thingy?*
 Come and see [HP System explained](https://t.me/HYPERx_UPDATES/3)
"""
